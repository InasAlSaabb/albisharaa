...
class DatabaseService {
  ...
  //   final database = await initDatabase();
  //   final results = await database.rawQuery(
  //       'SELECT verses.* FROM verses INNER JOIN trans ON verses.id = trans.id WHERE textch LIKE ? AND trans.tp = 1',
  //       ['%$searchText%']);
  //   return results.map((map) => VerseModel.fromJson(map)).toList();
  // }
  ...
  //   final database = await initDatabase();
  //   final results = await database.rawQuery(
  //       'SELECT verses.* FROM verses INNER JOIN trans ON verses.id = trans.id WHERE textch LIKE ? AND trans.tp = 2',
  //       ['%$searchText%']);
  //   return results.map((map) => VerseModel.fromJson(map)).toList();
}
